...
static bool    app_lv_scene_time = true;    /* 启停该检查功能 */
static bool    app_lv_scene_dlps = true;    /* 需要进入dlps */
static bool    app_lv_scene_back = true;    /* 需要回退界面 */
 ...
 */
void app_lv_scene_time_check_exec(bool status)
{
    app_lv_scene_time = status;
}
...
void app_lv_scene_time_check_over_time_update(uint8_t time)
{
    ...
    if (time == APP_LV_SCENE_OVER_TIME_MAX) {
        app_lv_scene_over_time_bak = APP_LV_SCENE_OVER_TIME_MAX;
        app_lv_scene_idle_time_bak = APP_LV_SCENE_IDLE_TIME_MAX;
    }
...
void app_lv_scene_time_check_idle_time_update(uint8_t time)
{
    ...
    if (time == APP_LV_SCENE_IDLE_TIME_MAX) {
        app_lv_scene_over_time_bak = APP_LV_SCENE_OVER_TIME_MAX;
        app_lv_scene_idle_time_bak = APP_LV_SCENE_IDLE_TIME_MAX;
    }
    app_lv_scene_time_check_reset();
...
{
    if (!app_lv_scene_time)
        return;
    /* 约减超时等待 */
    if (app_lv_scene_over_time != 0 &&
        app_lv_scene_over_time != APP_LV_SCENE_OVER_TIME_MAX)
        app_lv_scene_over_time--;
    /* 主界面超时等待结束发送休眠请求 */
    if (app_lv_scene_over_time == 0) {
        if (app_lv_scene_dlps) {
            app_lv_scene_dlps = false;
            if (!app_module_system_dlps_get())
                 app_module_system_dlps_set(true);
        } else {
            /* 约减空闲等待 */
            if (app_lv_scene_idle_time != 0 &&
                app_lv_scene_idle_time != APP_LV_SCENE_IDLE_TIME_MAX)
                app_lv_scene_idle_time--;
            /* 空闲等待结束回到主界面 */
            if (app_lv_scene_idle_time == 0) {
                if (app_lv_scene_back) {
                    app_lv_scene_back = false;
