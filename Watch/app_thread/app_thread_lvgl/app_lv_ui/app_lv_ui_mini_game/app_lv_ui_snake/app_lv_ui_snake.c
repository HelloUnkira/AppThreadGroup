...
static void app_lv_ui_snake_show(void *scene)
{
    if (app_lv_ui_res_local == NULL) {
        ...
        app_lv_ui_snake.self = app_lv_ui_res_local->scene;
        /* 禁用默认事件响应,事件重定向使用 */
        ...
        for (uint32_t idx = 0; idx < APP_LV_UI_SNAKE_LINE;     app_lv_ui_res_local->lbl_matrix_row_dsc[idx] = app_lv_ui_hor_pct(5), idx++);
        for (uint32_t idx = 0; idx < APP_LV_UI_SNAKE_ELEMENT;  app_lv_ui_res_local->lbl_matrix_col_dsc[idx] = app_lv_ui_ver_pct(5), idx++);
        /* 创建显示矩阵 */
        ...
        for (uint8_t idx1 = 0; idx1 < APP_LV_UI_SNAKE_LINE; idx1++)
        for (uint8_t idx2 = 0; idx2 < APP_LV_UI_SNAKE_ELEMENT; idx2++) {
            app_lv_ui_res_local->lbl_matrix[idx1][idx2] = app_lv_ui_style_btn_block(matrix, 1, 1, 1);
