 ...
 *@param[in] scene 场景
 */
static void app_lv_ui_watch_stop_show(void *scene)
{
    ...
    app_lv_scene_watch_stop.self = app_lv_ui_res_local == NULL ? NULL :
 ...
 *@param[in] scene 场景
 */
static void app_lv_ui_watch_stop_hide(void *scene)
{
    if (app_lv_ui_res_local != NULL) {
        /* 反初始化显示动画 */
...
app_lv_scene_t app_lv_scene_watch_stop = {
    /* 场景资源节点 */
    .show = app_lv_ui_watch_stop_show,
    .hide = app_lv_ui_watch_stop_hide,
};
