...
# 编写集成化源文件
def encode_app_sys_ext_src_table_c(file, file_list, json_list):
    # 写点简要的说明
    ...
    for index, json_text in enumerate(json_list):    # 字典列表中是字典
        ...
        file.write('};\n\n')
    # 生成资源总表
    file.write('static const app_sys_ext_src_t * app_sys_ext_src_set_table[] = {\n')
    for file_name in file_list:
        file.write('\tapp_sys_ext_src_%s_table,\n' % file_name)
    file.write('};\n\n')
    # 编写资源固化长度标签
    ...
    file.write('};\n\n')
    # 编写固化长度标签
    file.write('static const uint32_t app_sys_ext_src_table_size = %s;\n\n' % str(len(json_list)))
    # 编写固化访问函数
    ...
    file.write('const app_sys_ext_src_t * app_sys_ext_src_find_by_name')
    file.write('(const char *chunk_name, const char *data_name)\n')
    file.write('{\n')
    file.write('\tfor (uint32_t idx = 0; idx < app_sys_ext_src_table_size; idx++)\n')
    file.write('\t\tif (!strcmp(app_sys_ext_src_string_table[idx], chunk_name))\n')
    file.write('\t\t\tfor (uint32_t idx1 = 0; idx1 < app_sys_ext_src_set_size_table[idx]; idx1++)\n')
    file.write('\t\t\t\tif (!strcmp(app_sys_ext_src_set_table[idx][idx1].data_name, data_name))\n')
    file.write('\t\t\t\t\treturn &app_sys_ext_src_set_table[idx][idx1];\n')
...
# 启用集成化资源总表生成
def encode_app_sys_ext_src_table():
    ...
    app_sys_ext_src_table_c = open('app_sys_ext_src_table.c', mode='w', encoding='utf-8')
    # 解析
    encode_app_sys_ext_src_table_h(app_sys_ext_src_table_h)
    encode_app_sys_ext_src_table_c(app_sys_ext_src_table_c, file_list, json_list)
    # 关闭俩个文件
    app_sys_ext_src_table_h.close()
    app_sys_ext_src_table_c.close()
