...
typedef app_sys_list_dn_t app_sys_table_dn_t;
typedef app_sys_list_dl_t app_sys_table_dl_t;

typedef uint32_t (*app_sys_table_dt_fd_t)(app_sys_table_dn_t *node);
typedef bool     (*app_sys_table_dt_fc_t)(app_sys_table_dn_t *node1, app_sys_table_dn_t *node2);
typedef void     (*app_sys_table_dt_fv_t)(app_sys_table_dn_t *node,  uint32_t idx);

typedef struct {
    app_sys_table_dt_fd_t digest;   /* 摘要函数,散列函数 */
    app_sys_table_dt_fc_t confirm;  /* 比较函数 */
    app_sys_table_dt_fv_t visit;    /* 访问函数 */
    app_sys_table_dl_t   *list;     /* 散列链表 */
    uint32_t length;
} app_sys_table_dt_t;
