...
# 编写集成化源文件
def encode_app_sys_idx_str_c(file, xlsx_sheet, sheet_row, sheet_col):
    # 写点简要的说明
    ...
    file.write('void app_sys_idx_str_set_type(uint32_t type)\n')
    file.write('{\n\tapp_sys_idx_str_type = type;\n}\n\n')
    file.write('/*@brief获得多国语字符串\n')
    file.write(' *@param[in] index字符串编号(0~n-1)\n */\n')
    file.write('const char * app_sys_idx_str_get_str(uint32_t index)\n')
    file.write('{\n\treturn app_sys_idx_str_table[index][app_sys_idx_str_type];\n}\n')


# 编写集成化头文件
def encode_app_sys_idx_str_h(file, xlsx_sheet, sheet_row, sheet_col):
    file.write('#ifndef APP_SYS_IDX_STR_H\n')
    file.write('#define APP_SYS_IDX_STR_H\n\n')
    # 写点简要的说明
    file.write('/*一个代替注册的,脚本自动生成的,本地静态的字符串表\n')
    file.write(' *通过app_sys_idx_str.py生成\n')
    file.write(' *参考app_sys_idx_str.json中的模式生成源\n */\n\n')
...
# 启用集成化资源总表生成
def encode_app_sys_idx_str():
    # json转Python字符串并转标准字典
    json_file = open('app_sys_idx_str.json', 'r')
    json_dict = json.loads(json_file.read())
    json_file.close()
    # 仅支持批量化字符串表管理
    if json_dict['type'] != 'app_sys_idx_str':
        return
    # 从标准字典获取工作簿名字和单元
    ...
    # 解析
    encode_app_sys_idx_str_h(app_sys_idx_str_h, xlsx_sheet, sheet_row, sheet_col)
    encode_app_sys_idx_str_c(app_sys_idx_str_c, xlsx_sheet, sheet_row, sheet_col)
    # 关闭三个文件
    app_sys_idx_str_h.close()
    app_sys_idx_str_c.close()
    # 关闭工作簿
    xlsx_file.close()


if __name__ == '__main__':
    encode_app_sys_idx_str()
    print('app sys idx str generation completed')
