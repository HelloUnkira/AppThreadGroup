...
# 编写集成化源文件
def encode_app_sys_ext_mem_table_c(file, dir_list):
    # 写点简要的说明
    ...
    # 提取所有外源依赖
    file.write('static const app_sys_ext_mem_t app_sys_ext_mem_table[] = {\n')
    for items in dir_list:      # 列表中是字典
        file.write('\t{\n')
        for item in items:      # 从字典提取关键字和值
            if item == 'chunk_brief':
    ...
    file.write('};\n\n')
    # 编写固化访问函数
    file.write('/*@brief 通过名字索引chunk静态实例\n')
    file.write(' *@param name 名字\n')
    ...
    file.write('}\n\n')
    file.write('/*@brief 通过索引chunk静态实例\n')
...
# 编写集成化头文件
def encode_app_sys_ext_mem_table_h(file):
    file.write('#ifndef APP_SYS_EXT_MEM_TABLE_H\n')
    file.write('#define APP_SYS_EXT_MEM_TABLE_H\n\n')
    # 写点简要的说明
    file.write('/*一个代替注册的,脚本自动生成的,本地静态的外存chunk表\n')
    file.write(' *通过app_sys_ext_mem_table.py生成\n')
    file.write(' *参考app_sys_ext_mem_table.json中的模式生成源\n */\n\n')
...
# 启用集成化外存表生成
def encode_app_sys_ext_mem_table():
    # json转Python字符串并转标准字典
    json_file = open('app_sys_ext_mem_table.json', 'r')
    json_dict = json.loads(json_file.read())
    json_file.close()
    # 仅支持批量化外存表管理
    if json_dict['type'] != 'app_sys_ext_mem_table':
        return
    # 从标准字典获取标准列表
    dir_list = json_dict['app_sys_ext_mem_table']
    # 开启三个文件
    app_sys_ext_mem_table_h = open('app_sys_ext_mem_table.h', mode='w', encoding='utf-8')
    app_sys_ext_mem_table_c = open('app_sys_ext_mem_table.c', mode='w', encoding='utf-8')
    # 解析
    encode_app_sys_ext_mem_table_h(app_sys_ext_mem_table_h)
    encode_app_sys_ext_mem_table_c(app_sys_ext_mem_table_c, dir_list)
    # 关闭三个文件
    app_sys_ext_mem_table_h.close()
    app_sys_ext_mem_table_c.close()
