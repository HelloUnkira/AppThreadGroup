...
/* chunk field type: */
typedef enum {
    app_sys_heap_olsf_size_used = 0,    /* 压缩:最后一位作为Chunk使用标志位 */
    app_sys_heap_olsf_free_prev = 1,
    app_sys_heap_olsf_free_next = 2,
    app_sys_heap_olsf_size_near = 3,    /* 因为数组特性,这里默认指size left */
    app_sys_heap_olsf_type_num,
} app_sys_heap_olsf_type_t;

/* chunk field: */
#define app_sys_heap_olsf_field_declare(bits)           \
typedef struct {                                        \
    uint##bits##_t field[app_sys_heap_olsf_type_num];   \
} app_sys_heap_olsf_field_##bits##_t;                   \

app_sys_heap_olsf_field_declare(8);
app_sys_heap_olsf_field_declare(16);
app_sys_heap_olsf_field_declare(32);
app_sys_heap_olsf_field_declare(64);
