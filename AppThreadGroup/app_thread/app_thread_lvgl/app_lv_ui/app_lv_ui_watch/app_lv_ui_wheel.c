...
static void app_lv_ui_local_anim_handler(void *para, int32_t value)
{
    ...
    scene = app_lv_ui_wheel.float_t;
    APP_SYS_ASSERT(scene != NULL);
    APP_SYS_ASSERT(scene->self != NULL);
    APP_SYS_LOG_INFO("float_t(x:%d,y:%d,w:%d,h:%d)\n",
                        lv_obj_get_x(scene->self),
                        lv_obj_get_y(scene->self),
                        lv_obj_get_width(scene->self),
                        lv_obj_get_height(scene->self));
    scene = app_lv_ui_wheel.float_b;
    APP_SYS_ASSERT(scene != NULL);
    APP_SYS_ASSERT(scene->self != NULL);
...
static void app_lv_ui_wheel_show(void *scene)
{
    if (app_lv_ui_res_local == NULL) {
        ...
        app_lv_ui_wheel.self = app_lv_ui_res_local->scene;
        /* 初始化居中标签 */
        ...
        /* 浮动子窗口 */
        app_lv_ui_float_active(&app_lv_ui_wheel);
        /* 初始化显示动画 */
...
static void app_lv_ui_wheel_hide(void *scene)
{
    if (app_lv_ui_res_local != NULL) {
        ...
        /* 浮动子窗口 */
        app_lv_ui_float_hidden(&app_lv_ui_wheel);
        /* 反初始化场景 */
        lv_obj_del(app_lv_ui_res_local->scene);
        app_lv_ui_wheel.self = NULL;
