...
static char app_lv_ui_log_text[APP_SYS_LOG_TEXT_MAX + 1] = {0};
...
{
    memset(app_lv_ui_log_text, '\0', APP_SYS_LOG_TEXT_MAX + 1);
    if (!app_sys_log_text_peek(app_lv_ui_log_text))
        APP_SYS_LOG_WARN("can not take trace text");
        APP_SYS_LOG_INFO(app_lv_ui_log_text);
    return app_lv_ui_log_text;
}
