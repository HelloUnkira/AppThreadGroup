...
{
    char text[APP_SYS_TRACE_TEXT_MAX] = {0};
    snprintf(text, APP_SYS_TRACE_TEXT_MAX,
             "shutdown trace:");
    ...
    app_sys_trace_text_dump(text, true);
    /* 线程组工作时间: */
    snprintf(text, APP_SYS_TRACE_TEXT_MAX, "app thread group work time:%" PRIu64,
              app_module_clock_get_sec_tick());
    app_sys_trace_text_dump(text, true);
    #if APP_SYS_LOG_EXECUTE
    double execute_us = 0;
    app_thread_execute_us_get(app_thread_id_mix_irq, &execute_us);
    snprintf(text, APP_SYS_TRACE_TEXT_MAX, "app_thread_id_mix_irq:%.8lf",
            (double)execute_us / 1000000.0);
    app_sys_trace_text_dump(text, true);
    app_thread_execute_us_get(app_thread_id_mix_custom, &execute_us);
    snprintf(text, APP_SYS_TRACE_TEXT_MAX, "app_thread_id_mix_custom:%.8lf",
            (double)execute_us / 1000000.0);
    app_sys_trace_text_dump(text, true);
    app_thread_execute_us_get(app_thread_id_manage, &execute_us);
    snprintf(text, APP_SYS_TRACE_TEXT_MAX, "app_thread_id_manage:%.8lf",
            (double)execute_us / 1000000.0);
    app_sys_trace_text_dump(text, true);
    app_thread_execute_us_get(app_thread_id_lvgl, &execute_us);
    snprintf(text, APP_SYS_TRACE_TEXT_MAX, "app_thread_id_lvgl:%.8lf",
            (double)execute_us / 1000000.0);
    app_sys_trace_text_dump(text, true);
    #endif
    /* 关机的时间信息: */
    ...
    snprintf(text, APP_SYS_TRACE_TEXT_MAX, "dtime:%u-%u-%u;%u;%u:%u:%u",
