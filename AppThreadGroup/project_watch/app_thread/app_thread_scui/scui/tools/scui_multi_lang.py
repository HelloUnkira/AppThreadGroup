...
def encode_scui_multi_lang_c(file, xlsx_sheet, sheet_row, sheet_col):
    ...
    file.write('void scui_multi_lang_get(scui_multi_lang_type_t *type)\n{\n')
    file.write('\tSCUI_ASSERT(type != NULL);\n\t*type = scui_multi_lang_type;\n}\n\n')
    # 填充函数定义或者声明
    file.write('/*@brief 设置多国语语言类型\n')
    file.write(' *@param type 语言类型编号\n */\n\n')
    file.write('void scui_multi_lang_set(scui_multi_lang_type_t *type)\n{\n')
    ...
    file.write('const char * scui_multi_lang_str(scui_handle_t handle, scui_multi_lang_type_t type)')
    file.write('\n{\n\tscui_handle_t string = handle;\n')
    file.write('\tstring += SCUI_HANDLE_SYSTEM == type ? scui_multi_lang_type : type;\n')
    file.write('\tstring -= SCUI_MULTI_LANG_NUM_OFS;\n\treturn scui_handle_source(string);\n}\n')
