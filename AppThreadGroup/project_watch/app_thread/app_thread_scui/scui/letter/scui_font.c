...
{
    SCUI_ASSERT(utf8 == NULL);
    SCUI_ASSERT(unicode == NULL);
...
uint32_t scui_font_utf8_str_num(uint8_t *utf8)
{
    ...
    uint32_t str_num = 0;
    while (*utf8 != '\0') {
        uint8_t utf8_len = scui_font_utf8_bytes(*utf8);
        utf8 += utf8_len;
        str_num++;
    }
...
uint32_t scui_font_utf8_str_bytes(uint8_t *utf8)
{
    ...
    while (*utf8 != '\0') {
        uint8_t utf8_bytes = scui_font_utf8_bytes(*utf8);
        utf8 += utf8_bytes;
...
{
    uint32_t bytes_str    = scui_font_utf8_str_bytes(str);
    uint32_t bytes_suffix = scui_font_utf8_str_bytes(suffix);
...
uint32_t scui_font_unicode_by_utf8_str(uint32_t **unicode, uint8_t *utf8)
{
    ...
    /* 统计字符数量 */
    uint32_t utf8_num = scui_font_utf8_str_num(utf8);
    *unicode = SCUI_MEM_ALLOC(scui_mem_type_font, 4 * (utf8_num + 1));
    ...
    for (uint32_t idx = 0; idx < utf8_num; idx++) {
         uint8_t utf8_len = scui_font_utf8_to_unicode(utf8_ptr, &(*unicode)[idx]);
