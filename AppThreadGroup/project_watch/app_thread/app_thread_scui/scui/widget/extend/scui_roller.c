 ...
 *@param event 事件
 */
static void scui_roller_event(scui_event_t *event)
{
    switch (event->type) {
    ...
    case scui_event_draw_buffer: {
        ...
        case scui_roller_type_spin: {
            ...
            scui_size2_t size2 = {.w = scui_image_w(surface_image),.h = scui_image_h(surface_image),};
...
void scui_roller_invoke(scui_event_t *event)
{
    ...
    case scui_event_scroll_keep: {
        scui_handle_t handle = NULL;
        scui_scroll_center_get(widget->myself, &handle);
        
        roller->center = scui_widget_child_by_index(handle, 0);
        break;
    }
    default:
        break;
