...
{
    *move_type = scui_window_switch.move_type;
}
...
{
    scui_window_switch.move_type = move_type;
}
...
{
    *jump_type = scui_window_switch.jump_type;
}
...
{
    scui_window_switch.jump_type = jump_type;
}
...
{
    *jump_dir = scui_window_switch.jump_dir;
}
...
{
    scui_window_switch.jump_dir = jump_dir;
}
...
{
    SCUI_ASSERT(scui_window_switch.type != scui_window_switch_auto);
 ...
 *@param dir  窗口切换方向
 */
static void scui_window_switch_move_update(scui_window_switch_type_t type, scui_opt_dir_t dir)
{
    scui_window_switch.dir  = dir;
    scui_window_switch.type = type != scui_window_switch_auto ? type : scui_window_switch.move_type;
    
    /* 自适应需要更新窗口切换状态 */
    SCUI_ASSERT(scui_window_switch.dir  != scui_opt_dir_none);
    SCUI_ASSERT(scui_window_switch.type != scui_window_switch_auto);
}
 ...
 *@param dir  窗口切换方向
 */
static void scui_window_switch_jump_update(scui_window_switch_type_t type, scui_opt_dir_t dir)
{
    scui_window_switch.dir  = dir  != scui_opt_dir_none       ? dir  : scui_window_switch.jump_dir;
    scui_window_switch.type = type != scui_window_switch_auto ? type : scui_window_switch.jump_type;
...
{
    /* 动画速度[ptr,enc,bar,key,auto,jump] */
    scui_handle_t anima_type = scui_window_switch.anima_type;
    scui_coord_t anima_speed[] = SCUI_WINDOW_SWITCH_MOVE_SPD;
    anima->period = anima->period * 1000 / anima_speed[anima_type];
    ...
    case 5: anima->path = scui_map_linear;      break;
    }
...
static void scui_window_move_anima_state(uint8_t type)
{
    ...
    case 0x00: {
        ...
        if (!scui_window_switch.over_scroll) break;
        scui_window_switch.over_scroll = false;
    ...
    case 0x01: {
        if (scui_window_switch.over_scroll) break;
        scui_window_switch.over_scroll = true;
...
static void scui_window_move_anima_auto(int32_t value_s, int32_t value_e, int32_t value_all, uint32_t period)
{
    ...
    /* 确定当前动画的路径/周期 */
    scui_window_switch_anima(&anima);
 ...
 *@param anima_type 动画类型
 *@retval 成功失败
 */
static bool scui_window_switch_event_catch(scui_event_t *event, scui_opt_dir_t event_dir, scui_handle_t anima_type)
{
    ...
    if (target != SCUI_HANDLE_INVALID && event_dir != scui_opt_dir_none) {
        ...
        /* 更新动画类型,交互方向 */
        scui_window_switch.anima_type = anima_type;
        scui_window_switch_move_update(switch_type, event_dir);
        /* 就绪窗口资源 */
...
void scui_window_switch_event(scui_event_t *event)
{
    ...
    case scui_event_ptr_up: {
        if (scui_window_switch.lock_move) {
            scui_window_switch.lock_move = false;
            scui_window_switch.anima_type = 4;
    ...
    case scui_event_key_click: {
        ...
        /* 给定方向进行窗口切换 */
        scui_window_switch.pos = scui_opt_pos_all;
        if (scui_window_switch_event_catch(event, event_dir, 3)) {
            scui_window_move_anima_inout(scui_window_switch.list[0], false);
            scui_event_mask_over(event);
        }
...
bool scui_window_switch_jump(scui_handle_t handle, scui_window_switch_type_t type, scui_opt_dir_t dir)
{
    ...
    /* 自适应需要更新窗口切换状态 */
    scui_window_switch_jump_update(type, dir);
    ...
    scui_window_switch.list[1] = handle;
    /* 更新动画类型 */
    scui_window_switch.anima_type = 5;
    /* 就绪窗口资源 */
    scui_window_switch_ready();
    /* 额外做一次校正 */
    scui_point_t point = scui_window_switch.point;
