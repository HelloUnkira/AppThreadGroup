 ...
 *@retval 成功失败
 */
static bool scui_widget_draw_clip(scui_handle_t handle, scui_area_t *clip, scui_handle_t type)
{
    /* 不给无效目标添加剪切域 */
    ...
    case 0: {
        ...
        if (clip  !=  NULL) {
            ...
            scui_area_t clip_r = widget->clip;
            clip_w.x += clip_r.x;
            clip_w.y += clip_r.y;
            if (!scui_area_inter2(&clip_w, &clip_r))
                 return false;
        }
...
void scui_widget_draw(scui_handle_t handle, scui_area_t *clip, bool sync, scui_handle_t type)
{
    ...
    if (sync) {
        /* 移除跟主窗口相关所有绘制事件 */
        scui_event_define(event, handle_t, false, scui_event_draw, NULL);
...
static void scui_widget_show_async(scui_handle_t handle)
{
    ...
    scui_widget_draw(widget->myself, NULL, only, 2);
}
...
static void scui_widget_hide_async(scui_handle_t handle)
{
    ...
    if (widget->parent != SCUI_HANDLE_INVALID) {
        bool only = scui_widget_surface_only(widget);
        scui_widget_draw(widget->parent, NULL, only, 2);
    }
...
void scui_widget_event_dispatch(scui_event_t *event)
{
    ...
    case scui_event_draw_ready: {
        ...
        if (widget->parent == SCUI_HANDLE_INVALID) {
            ...
            if (!surface_only &&  event->style.sync) {
                ...
                scui_widget_surface_remap(widget, surface_fb);
            }
            
            /* 绘制起始, 更新surface剪切域 */
            /* 为所有控件及其子控件添加全局剪切域 */
            /* scui_widget_clip_check(widget->myself, true); */
            scui_widget_clip_reset(widget, NULL);
    ...
    default: {
        ...
        event->object = scui_widget_tree(handle);
        scui_widget_event_process(event);
        event->object = handle;
        return;
    }
