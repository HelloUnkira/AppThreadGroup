...
// 纯偷懒
static inline void * gifdec_alloc(size_t size)
{
    SCUI_LOG_WARN("gifdec cache: %u", size);
    return SCUI_MEM_ALLOC(scui_mem_type_graph, (uint32_t)size);
}

static inline void gifdec_free(void *ptr)
{
    SCUI_MEM_FREE(ptr);
}
...
{
    SCUI_LOG_ERROR("unsupport this way");
    SCUI_ASSERT(false);
...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    if(memcmp(sigver, "GIF", 3) != 0) {
        SCUI_LOG_WARN("invalid signature");
        goto fail;
    }
    /* Version */
    f_gif_read(gif_base, sigver, 3);
    if(memcmp(sigver, "89a", 3) != 0) {
        SCUI_LOG_WARN("invalid version");
        goto fail;
    }
    /* Width x Height */
    ...
    /* Presence of GCT */
    if(!(fdsz & 0x80)) {
        SCUI_LOG_WARN("no global color table");
        goto fail;
    }
    /* Color Space's Depth */
    depth = ((fdsz >> 4) & 7) + 1;
    ...
    if(0 == width || 0 == height){
        SCUI_LOG_WARN("Zero size image");
        goto fail;
    }
#if 1
    if(0 == (INT_MAX - sizeof(gd_GIF) - LZW_CACHE_SIZE) / width / height / 5){
        SCUI_LOG_WARN("Image dimensions are too large");
        goto fail;
    } 
    gif = gifdec_alloc(sizeof(gd_GIF) + 5 * width * height + LZW_CACHE_SIZE);
    #else
    if(0 == (INT_MAX - sizeof(gd_GIF)) / width / height / 5){
        SCUI_LOG_WARN("Image dimensions are too large");
        goto fail;
    } 
    gif = gifdec_alloc(sizeof(gd_GIF) + 5 * width * height);
    #endif
    if(!gif) goto fail;
...
#endif
    gif->anim_start = f_gif_seek(gif, 0, 1);
    gif->loop_count = -1;
    goto ok;
fail:
...
read_plain_text_ext(gd_GIF * gif)
{
    if(gif->plain_text) {
        ...
        uint8_t cw, ch, fg, bg;
        size_t sub_block;
        f_gif_seek(gif, 1, 1); /* block size = 12 */
        tx = read_num(gif);
        ty = read_num(gif);
        tw = read_num(gif);
        th = read_num(gif);
        ...
        f_gif_read(gif, &fg, 1);
        f_gif_read(gif, &bg, 1);
        sub_block = f_gif_seek(gif, 0, 1);
        gif->plain_text(gif, tx, ty, tw, th, cw, ch, fg, bg);
        f_gif_seek(gif, sub_block, 0);
    }
    else {
...
{
    if(gif->comment) {
        size_t sub_block = f_gif_seek(gif, 0, 1);
        gif->comment(gif);
        f_gif_seek(gif, sub_block, 0);
    }
...
read_ext(gd_GIF * gif)
{
    ...
    switch(label) {
        ...
        case 0xFF:
            ...
            break;
        default:
            SCUI_LOG_WARN("unknown extension: %02X\n", label);
    }
}
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    start = f_gif_seek(gif, 0, 1);
    discard_sub_blocks(gif);
    end = f_gif_seek(gif, 0, 1);
    f_gif_seek(gif, start, 0);
    ...
    while (frm_off < frm_size) {
        ...
        while (sp > p_stack) {
            if(frm_off >= frm_size){
                SCUI_LOG_WARN("LZW table token overflows the frame buffer");
                return -1;
            }
            *ptr++ = *(--sp);
            frm_off += 1;
...
new_table(int key_size)
{
    ...
    Table * table = gifdec_alloc(sizeof(*table) + sizeof(Entry) * init_bulk);
    if(table) {
...
read_image(gd_GIF * gif)
{
    ...
    if(gif->fx + (uint32_t)gif->fw > gif->width || gif->fy + (uint32_t)gif->fh > gif->height){
        SCUI_LOG_WARN("Frame coordinates out of image bounds");
        return -1;
    }
...
gd_get_frame(gd_GIF * gif)
{
    ...
    while(sep != ',') {
        if(sep == ';') {
            f_gif_seek(gif, gif->anim_start, 0);
...
static int f_gif_seek(gd_GIF * gif, size_t pos, int k)
{
    if(gif->is_file) {
        ...
        if(k == 1) gif->f_rw_p += pos;
        else if(k == 0) gif->f_rw_p = pos;
        return gif->f_rw_p;
    }
}
