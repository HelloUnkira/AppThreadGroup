...
def get_filled_circle_info(radius):
    print("circle res with radius", radius)
    width = radius * 2 + 1
    height = radius * 2 + 1
        ...
        with open(outfilename,"w") as f:
            ...
            for i in args.spec:
                ...
                for info in data_info_array:
                    ...
                    f.write(("    {.offset = %4d, .count = %4d, .pixel = %4d}, \n" % (start_offset, valid_count, data_value_offset)))

                    row_index += 1
                print('};', file=f)

                print('static const uint8_t %s_data_sub_%d[%d] = {' % (name, i, len(data_value_array)), file=f)
                lineWidth=0
