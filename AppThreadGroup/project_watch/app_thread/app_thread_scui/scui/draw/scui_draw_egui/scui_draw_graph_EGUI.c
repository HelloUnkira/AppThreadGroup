...
static void scui_draw_crect(scui_draw_dsc_t *draw_dsc)
{
    ...
    draw_graph_circle.src_center.x = dst_area.x + src_radius;
    draw_graph_circle.src_center.y = dst_area.y + src_radius;
...
static void scui_draw_shadow(scui_draw_dsc_t *draw_dsc)
{
    ...
    for (scui_coord_t idx_width = 0; idx_width < src_width; idx_width += lvl_width) {
        ...
        draw_graph_circle.src_alpha = (uint16_t)idx_width * src_alpha / src_width;
        draw_graph_circle.src_width  = src_radius - idx_width < lvl_width ?
                                       src_radius - idx_width : lvl_width;
        draw_graph_circle.src_radius = src_radius - idx_width;
    ...
    for (scui_coord_t idx_w = 0; idx_w < w; idx_w += lvl_w) {
        draw_graph_line.src_alpha = (uint16_t)idx_w * src_alpha / w;
...
static void scui_draw_arc(scui_draw_dsc_t *draw_dsc)
{
    ...
    draw_graph_arc.src_angle_s = src_angle_s;
    draw_graph_arc.src_angle_e = src_angle_e;
    ...
    do {
        ...
        if (src_angle_s < 180) {
            draw_graph_arc.src_angle_e = 90 - (src_angle_s - 90);
            draw_graph_arc.src_angle_s = 90 - (src_angle_e - 90);
            scui_draw_arc_corner(&draw_graph_arc, scui_draw_circle_type_lb);
        }
        if (src_angle_s < 270) {
            draw_graph_arc.src_angle_s = src_angle_s - 180;
            draw_graph_arc.src_angle_e = src_angle_e - 180;
            scui_draw_arc_corner(&draw_graph_arc, scui_draw_circle_type_lt);
        }
        if (src_angle_s < 360) {
            draw_graph_arc.src_angle_e = 90 - (src_angle_s - 270);
            draw_graph_arc.src_angle_s = 90 - (src_angle_e - 270);
