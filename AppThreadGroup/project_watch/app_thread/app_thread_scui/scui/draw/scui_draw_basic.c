...
void scui_draw_area_fill(scui_surface_t *dst_surface, scui_area_t *dst_clip, SCUI_PIXEL_TYPE *pixel)
{
    ...
    scui_coord_t dis_line = dst_surface->clip.w * SCUI_PIXEL_SIZE;
    /* 先填充第一行像素点 */
    for (scui_coord_t idx = 0; idx < draw_area.w; idx++)
        app_sys_mem_w(dst_addr + idx * SCUI_PIXEL_SIZE, *pixel, SCUI_PIXEL_TYPE);
    /* 后使用第一行像素点填充其他行像素点 */
    for (scui_coord_t idx = 1; idx < draw_area.h; idx++)
        scui_draw_line_copy(dst_addr + (idx + draw_area.y) * dis_line, dst_addr, draw_area.w);
}
                         ...
                         scui_surface_t *src_surface, scui_area_t *src_clip)
{
    ...
    for (scui_coord_t idx = 0; idx < draw_area.h; idx++)
        scui_draw_line_copy(dst_addr + (idx + dst_clip_v.y) * dis_line,
                            src_addr + (idx + src_clip_v.y) * dis_line, draw_area.w);
}
                          ...
                          scui_surface_t *src_surface, scui_area_t *src_clip)
{
    ...
    for (scui_coord_t idx_item = 0; idx_item < draw_area.w; idx_item++) {
        ...
        pixel = scui_pixel_blend_with_alpha((SCUI_PIXEL_TYPE *)src_addr_ofs, src_surface->alpha,
                                            (SCUI_PIXEL_TYPE *)dst_addr_ofs, dst_surface->alpha);
