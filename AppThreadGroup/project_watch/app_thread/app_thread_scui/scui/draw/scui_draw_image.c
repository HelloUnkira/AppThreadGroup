...
void scui_draw_ctx_image_scale(scui_draw_dsc_t *draw_dsc)
{
    ...
    scui_point_t   *dst_anchor  = &draw_dsc->image_scale.dst_anchor;
    scui_point_t   *src_center  = &draw_dsc->image_scale.src_center;
    /* draw dsc args<e> */
    /* */
    SCUI_ASSERT(dst_surface != NULL && dst_surface->pixel != NULL && dst_clip != NULL);
    SCUI_ASSERT(src_image != NULL && src_clip != NULL);
    ...
    #if 1
    scui_draw_area_scale_blend(true, dst_surface, *dst_clip,
        &image_surface, *src_clip, src_color,
        *src_scale, *dst_anchor, *src_center);
    #else
...
void scui_draw_ctx_image_rotate(scui_draw_dsc_t *draw_dsc)
{
    ...
    scui_point_t   *dst_anchor  = &draw_dsc->image_rotate.dst_anchor;
    ...
    #if 1
    scui_draw_area_rotate_blend(true, dst_surface, *dst_clip,
        &image_surface, *src_clip, src_color,
        src_angle, *dst_anchor, *src_center);
    #else
    /* 矩阵是万能的图形变换 */
    scui_matrix_t src_matrix = {0};
    scui_matrix_identity(&src_matrix);
