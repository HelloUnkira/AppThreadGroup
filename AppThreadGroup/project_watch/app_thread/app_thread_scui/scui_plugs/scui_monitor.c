 ...
 *@param instance 动画实例
 */
static void scui_monitor_anima_expire(void *instance)
{
...
void scui_monitor_show(bool buffer)
{
    if (scui_monitor.window == SCUI_HANDLE_INVALID) {
        ...
        anima.expire = scui_monitor_anima_expire;
        anima.period = SCUI_ANIMA_TICK * 2;
        anima.reload = SCUI_ANIMA_INFINITE;
        scui_anima_create(&anima, &scui_monitor.anima);
        scui_anima_start(scui_monitor.anima);
    }
}
...
void scui_monitor_hide(void)
{
    if (scui_monitor.window != SCUI_HANDLE_INVALID) {
        ...
        scui_anima_stop(scui_monitor.anima);
        scui_anima_destroy(scui_monitor.anima);
        scui_monitor.anima = SCUI_HANDLE_INVALID;
