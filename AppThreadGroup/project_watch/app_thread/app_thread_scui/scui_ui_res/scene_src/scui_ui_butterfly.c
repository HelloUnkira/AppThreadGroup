...
void scui_ui_scene_butterfly_event_proc(scui_event_t *event)
{
    ...
    case scui_event_show:
        ...
        if (scui_widget_event_check_prepare(event)) {
            ...
            scui_ui_res_local = SCUI_MEM_ALLOC(scui_mem_type_none, sizeof(*scui_ui_res_local));
...
void scui_ui_scene_butterfly_custom_event_proc(scui_event_t *event)
{
    ...
    case scui_event_draw:
        ...
        if (scui_widget_event_check_execute(event)) {
            ...
            scui_matrix_t *matrix_wing = scui_ui_res_local->matrix_wing;
            scui_widget_surface_draw_image_by_matrix(widget, image_wing[1], NULL, &matrix_wing[2]);
            scui_widget_surface_draw_image_by_matrix(widget, image_wing[1], NULL, &matrix_wing[3]);
            scui_widget_surface_draw_image_by_matrix(widget, image_wing[0], NULL, &matrix_wing[0]);
            scui_widget_surface_draw_image_by_matrix(widget, image_wing[0], NULL, &matrix_wing[1]);
            
            /* 触角 */
            scui_handle_t  image_antenna  = scui_ui_res_local->image_antenna;
            scui_matrix_t *matrix_antenna = scui_ui_res_local->matrix_antenna;
            scui_widget_surface_draw_image_by_matrix(widget, image_antenna, NULL, &matrix_antenna[0]);
            scui_widget_surface_draw_image_by_matrix(widget, image_antenna, NULL, &matrix_antenna[1]);
            
            /* 躯干 */
            scui_handle_t  image_trunk  = scui_ui_res_local->image_trunk;
            scui_matrix_t *matrix_trunk = scui_ui_res_local->matrix_trunk;
            scui_widget_surface_draw_image_by_matrix(widget, image_trunk, NULL, &matrix_trunk[0]);
            scui_widget_surface_draw_image_by_matrix(widget, image_trunk, NULL, &matrix_trunk[1]);
        }
        break;
