/*实现目标:
 *    窗口:scui_test_ui_scroll
 *    用途: 测试不同 scroll 构建配置(销毁重建刷新)
 *    初始化: 采用配置0(垂直滚动)
 *    交互: 每敲击一次按键, 销毁 scroll 并采用下一种构建配置重建
 *    配置0: 垂直滚动(循环,翻1页,编码器/按键滚动一个条目)
 *    配置1: 水平滚动(无回弹,翻1条目,编码器/按键滚动一个条目)
 *    配置2: 自由滚动(子控件为小正方形,有间隔)
 *    配置3: 滚动套滚动(垂直滚动内嵌水平滚动,子控件宽度不一,边缘对齐粘连)
 */
