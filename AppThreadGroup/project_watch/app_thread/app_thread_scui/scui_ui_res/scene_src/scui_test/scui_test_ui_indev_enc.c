...
#define SCUI_UI_INDEV_ENC_KEEP      10  /* 保留打印行数 */
#define SCUI_UI_INDEV_ENC_MARK_LEN  48  /* 单行标记长度 */

static struct {
    char  (*line)[SCUI_UI_INDEV_ENC_MARK_LEN];  /* 标记序列 */
    char   *text_str;                           /* 文本缓冲 */
 ...
 *@param mark 标记文本
 */
static void scui_test_ui_indev_enc_print(const char *mark)
{
    /* 行数达到上限,前移丢弃最旧 */
    if (scui_ui_res_local->line_cnt >= SCUI_UI_INDEV_ENC_KEEP) {
        for (uint8_t idx = 1; idx < SCUI_UI_INDEV_ENC_KEEP; idx++)
            memcpy(scui_ui_res_local->line[idx - 1],
                scui_ui_res_local->line[idx], SCUI_UI_INDEV_ENC_MARK_LEN);
    } else {
        scui_ui_res_local->line_cnt++;
    }
    /* 新建标记写入尾部 */
    snprintf(scui_ui_res_local->line[scui_ui_res_local->line_cnt - 1],
        SCUI_UI_INDEV_ENC_MARK_LEN, "%s", mark);
    
    /* 拼接文本, 换行分隔 */
    char *str = scui_ui_res_local->text_str;
    char *end = scui_ui_res_local->text_str +
        SCUI_UI_INDEV_ENC_KEEP * SCUI_UI_INDEV_ENC_MARK_LEN;
    ...
    scui_string_update_str(SCUI_UI_SCENE_TEST_UI_INDEV_ENC_2,
        (uint8_t *)scui_ui_res_local->text_str);
}

/*@brief 长条事件回调(click喂入enc, enc事件打印)
 *@param event 事件
 */
void scui_test_ui_indev_enc_strip_event_proc(scui_event_t *event)
{
    switch (event->type) {
    case scui_event_ptr_click: {
        ...
        /* 注入enc输入数据: 上半正转(向上), 下半反转(向下) */
        scui_indev_data_t indev_data = {0};
        indev_data.type         = scui_indev_type_enc;
        indev_data.enc.enc_diff = point.y < SCUI_VER_RES / 3 ? 1 : -1;
    ...
    case scui_event_enc_tick: {
        ...
        scui_test_ui_indev_enc_print(mark);
        break;
    }
    default:
        break;
 ...
 *@param event 事件
 */
void scui_test_ui_indev_enc_event_proc(scui_event_t *event)
{
    switch (event->type) {
