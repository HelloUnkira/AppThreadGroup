...
void scui_ui_scene_soccer_event_proc(scui_event_t *event)
{
    ...
    case scui_event_show:
        ...
        if (scui_widget_event_check_prepare(event)) {
            ...
            scui_ui_res_local->image_bg = scui_image_prj_image_src_Soccer_boardpng;
            ...
            for (uint8_t idx_i = 0; idx_i < 5; idx_i++) {
                ...
                if ((idx_j * 5 + idx_i) >= scui_image_prj_image_src_Soccer_starpng - 2 - image_fg_0)
                    scui_ui_res_local->image_fg[idx_j][idx_i] += 1;
                if ((idx_j * 5 + idx_i) >= scui_image_prj_image_src_Soccer_backgroundpng - image_fg_0)
