...
lvgl_view_dist(lvgl_view_sched.touch_last.x, lvgl_view_sched.touch_curr.x);
    lv_coord_t dist_y = lvgl_view_dist(lvgl_view_sched.touch_last.y, lvgl_view_sched.touch_curr.y);
    ...
    if (dist_x > dist_y) {
        if (lvgl_view_sched.touch_last.x > lvgl_view_sched.touch_curr.x) return LV_DIR_LEFT;
        if (lvgl_view_sched.touch_last.x < lvgl_view_sched.touch_curr.x) return LV_DIR_RIGHT;
    }
    
    if (dist_y > dist_x) {
        if (lvgl_view_sched.touch_last.y > lvgl_view_sched.touch_curr.y) return LV_DIR_TOP;
        if (lvgl_view_sched.touch_last.y < lvgl_view_sched.touch_curr.y) return LV_DIR_BOTTOM;
    }
...
static bool lvgl_view_obj_scrollable(lv_obj_t *obj, lv_dir_t dir, lv_obj_t **obj_scrollable, lv_coord_t *obj_dist)
{
    // 滚动条件已经达成
    if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLLABLE) &&
...
// 向上冒泡到父控件
bool lvgl_view_obj_bubble_scrollable(lv_obj_t *obj, lv_dir_t dir, lv_obj_t **obj_scrollable, lv_coord_t *obj_dist)
{
...
// 向下沉底到子控件
bool lvgl_view_obj_tree_scrollable(lv_obj_t *obj, lv_dir_t dir, lv_obj_t **obj_scrollable, lv_coord_t *obj_dist)
{
    if (lvgl_view_obj_scrollable(obj, dir, obj_scrollable, obj_dist))
        return true;
...
static void lvgl_view_src_make(void)
{
    ...
    lv_snapshot_take_to_buf(lvgl_view_sched.view_c, LV_IMG_CF_TRUE_COLOR, &lvgl_view_sched.view_sps_c, lvgl_view_sched.buffer_c, lvgl_view_sched.buffer_size);
    lv_snapshot_take_to_buf(lvgl_view_sched.view_t, LV_IMG_CF_TRUE_COLOR, &lvgl_view_sched.view_sps_t, lvgl_view_sched.buffer_t, lvgl_view_sched.buffer_size);
...
static void lvgl_view_slide_anim_exec_cb(void *var, int32_t val)
{
    ...
    if (lvgl_view_sched.slide_dir == LV_DIR_TOP)    point_t.y += LV_VER_RES;
    if (lvgl_view_sched.slide_dir == LV_DIR_BOTTOM) point_t.y -= LV_VER_RES;
    if (lvgl_view_sched.slide_dir == LV_DIR_LEFT)   point_t.x += LV_HOR_RES;
    if (lvgl_view_sched.slide_dir == LV_DIR_RIGHT)  point_t.x -= LV_HOR_RES;
...
static void lvgl_view_slide_anim_ready_cb(lv_anim_t *a)
{
    ...
    if (lvgl_view_sched.pct == 0) {
        ...
        lvgl_view_config_t *config = lvgl_view_id_inst(lvgl_view_sched.page_t);
...
static void lvgl_view_slide_anim_auto(int32_t val_s, int32_t val_e, int32_t val_all, int32_t period)
{
    // 将之前的动画删除掉
...
/*@brief 滑动监听事件回调
 */
static void lvgl_view_sched_slide_listener_event_cb(lv_event_t* e)
{
    ...
    case LV_EVENT_PRESSING: {
        ...
        if (!lvgl_view_sched.move_lock) {
            ...
            if (lvgl_view_sched.page_c != 0 && slide_dir != LV_DIR_NONE) {
                ...
                lvgl_view_config_t *config_c = lvgl_view_id_inst(lvgl_view_sched.page_c);
                ...
                if (config_c->page_r != 0 && slide_dir == LV_DIR_LEFT) {
                    ...
                    config_t = lvgl_view_id_inst(config_c->page_r);
                }
                if (config_c->page_l != 0 && slide_dir == LV_DIR_RIGHT) {
                    ...
                    config_t = lvgl_view_id_inst(config_c->page_l);
                }
                if (config_c->page_t != 0 && slide_dir == LV_DIR_BOTTOM) {
                    ...
                    config_t = lvgl_view_id_inst(config_c->page_t);
                }
                if (config_c->page_b != 0 && slide_dir == LV_DIR_TOP) {
                    ...
                    config_t = lvgl_view_id_inst(config_c->page_b);
                }
                ...
                if (config_t != NULL) {
                    ...
                    lvgl_view_sched.point_c = (lv_point_t){0};
                    lvgl_view_sched.point_t = (lv_point_t){0};
        ...
        if (lvgl_view_sched.move_lock) {
            ...
            /* 位移风格:动画只补充移位 */
            lv_point_t offset = {0};
            offset.x = point_c.x + lvgl_view_sched.touch_curr.x - lvgl_view_sched.touch_dist.x;
            offset.y = point_c.y + lvgl_view_sched.touch_curr.y - lvgl_view_sched.touch_dist.y;
            /* 将上一次动画中没有用完的位移取出 */
            if (lvgl_view_sched.anim != 0) {
                ...
                if ((lvgl_view_sched.slide_dir & LV_DIR_HOR) != 0) offset.x += val_e - val_c;
                if ((lvgl_view_sched.slide_dir & LV_DIR_VER) != 0) offset.y += val_e - val_c; 
            }
...
bool lvgl_view_sched_jump(uint32_t page, uint32_t transform_type, lv_dir_t dir)
{
    ...
    if (lvgl_view_sched.anim_lock)  {LV_LOG_WARN("view switch"); return false;}
    if (lvgl_view_sched.move_lock)  {LV_LOG_WARN("view switch"); return false;}
    if (lvgl_view_sched.jump_lock)  {LV_LOG_WARN("view switch"); return false;}
    ...
    if (lvgl_view_sched.page_c == 0 || lvgl_view_sched.page_c == page ||
        ...
        lvgl_view_sched_config_page(page);
        lvgl_view_sched.jump_lock = false;
        return true;
    }
    ...
    /* 目标显示未创建, 创建目标显示 */
    lvgl_view_config_t *config_t = lvgl_view_id_inst(page);
    if (lvgl_view_sched.view_t == NULL) {
        lv_obj_t *view = lv_obj_create(lvgl_view_sched.view_root);
        if (lvgl_view_sched.view_event_cb != NULL)
...
void lvgl_view_sched_ready(void)
{
    ...
    lv_obj_add_flag(lvgl_view_sched.view_root, LV_OBJ_FLAG_EVENT_BUBBLE);
    ...
    lv_obj_add_event_cb(lvgl_view_sched.view_root, lvgl_view_sched_slide_listener_event_cb, LV_EVENT_ALL, NULL);
    ...
    // 备注:::这里有必要优化, buffer_n可以直达送县的buffer的,暂未继续深入研究
    size_t buffer_size = lv_snapshot_buf_size_needed(lvgl_view_sched.view_root, LV_IMG_CF_TRUE_COLOR);
    ...
    // 画布绑定输出常驻并且隐藏
    lv_canvas_set_buffer(lvgl_view_sched.view_cvs, lvgl_view_sched.buffer_n, LV_HOR_RES, LV_VER_RES, LV_IMG_CF_TRUE_COLOR);
    lv_obj_add_flag(lvgl_view_sched.view_cvs, LV_OBJ_FLAG_HIDDEN);
    #endif
}

void lvgl_view_sched_view_event_cb_register(lv_event_cb_t view_event_cb)
{
    lvgl_view_sched.view_event_cb = view_event_cb;
}

void lvgl_view_sched_config_page(uint32_t page)
{
    if (lvgl_view_sched.page_c != page) {
    if (lvgl_view_sched.view_c != NULL) {
        lvgl_view_config_t *config = lvgl_view_id_inst(lvgl_view_sched.page_c);
        lv_obj_t *page_c = lv_obj_get_child(lvgl_view_sched.view_c, 0);
        config->destroy(page_c);
