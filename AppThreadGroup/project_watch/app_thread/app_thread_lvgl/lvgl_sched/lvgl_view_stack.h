...
bool lvgl_view_stack_reset_by(uint32_t page, bool reserve, uint32_t transform_type, lv_dir_t dir);
bool lvgl_view_stack_goback_by(uint32_t page, uint32_t transform_type, lv_dir_t dir);
bool lvgl_view_stack_cover_by(uint32_t page, uint32_t transform_type, lv_dir_t dir);
bool lvgl_view_stack_add_by(uint32_t page, bool reserve, uint32_t transform_type, lv_dir_t dir);
bool lvgl_view_stack_del_by(uint32_t page, uint32_t transform_type, lv_dir_t dir);

bool lvgl_view_stack_add_prio_by(uint32_t page, bool reserve, uint32_t transform_type, lv_dir_t dir,
...
static inline bool lvgl_view_stack_add_prio(uint32_t page, bool reserve, bool (*prio_compare)(uint32_t page, uint32_t page_new))
{
    return lvgl_view_stack_add_prio_by(page, reserve, lvgl_view_tr_auto, LV_DIR_NONE, prio_compare);
}



/* 额外扩充语义, 将add,del融合在一起的效果, 可以不使用层级管理理念设计界面层级跳转 */
static inline bool lvgl_view_stack_jump_by(uint32_t page, uint32_t transform_type, lv_dir_t dir)
{
