...
static void lvgl_view_transform_move(void)
{
    ...
    if ((lvgl_view_sched.slide_dir & LV_DIR_HOR) != 0) {
        // x支持负偏移以得到正确计算
        lv_canvas_transform(lvgl_view_sched.view_cvs, &lvgl_view_sched.view_sps_c, 0, LV_IMG_ZOOM_NONE,
            lvgl_view_sched.point_c.x, lvgl_view_sched.point_c.y, 0, 0, true);
        lv_canvas_transform(lvgl_view_sched.view_cvs, &lvgl_view_sched.view_sps_t, 0, LV_IMG_ZOOM_NONE,
        ...
        if (point_c_y < 0) {
            ...
            int32_t skip_line_bytes = skip_line_px * lv_img_cf_get_px_size(view_sps_c.header.cf);
        ...
        if (point_t_y < 0) {
            ...
            int32_t skip_line_bytes = skip_line_px * lv_img_cf_get_px_size(view_sps_t.header.cf);
    ...
    lv_canvas_draw_img(lvgl_view_sched.view_cvs, lvgl_view_sched.point_c.x, lvgl_view_sched.point_c.y,
        &lvgl_view_sched.view_sps_c, &draw_dsc_c);
    
    lv_draw_img_dsc_t draw_dsc_t = {0};
    lv_draw_img_dsc_init(&draw_dsc_t);
    lv_canvas_draw_img(lvgl_view_sched.view_cvs, lvgl_view_sched.point_t.x, lvgl_view_sched.point_t.y,
        &lvgl_view_sched.view_sps_t, &draw_dsc_t);
    #endif
    
    #else
    lv_obj_set_pos(lvgl_view_sched.view_c, lvgl_view_sched.point_c.x, lvgl_view_sched.point_c.y);
    lv_obj_set_pos(lvgl_view_sched.view_t, lvgl_view_sched.point_t.x, lvgl_view_sched.point_t.y);
    #endif
...
{
    #if LVGL_VIEW_CVS_USE
    lv_coord_t zoom_c = lv_map(lvgl_view_sched.pct, 0, 100, LV_IMG_ZOOM_NONE, LV_IMG_ZOOM_NONE / 2);
    lv_coord_t zoom_t = lv_map(lvgl_view_sched.pct, 0, 100, LV_IMG_ZOOM_NONE / 2, LV_IMG_ZOOM_NONE);
    ...
    if (lvgl_view_sched.transform_type == lvgl_view_tr_zoom1) {
        ...
        lv_canvas_transform(lvgl_view_sched.view_cvs, &lvgl_view_sched.view_sps_t, 0, zoom_t,
            lvgl_view_sched.point_t.x, lvgl_view_sched.point_t.y,
            lvgl_view_sched.view_sps_t.header.w / 2, lvgl_view_sched.view_sps_t.header.h / 2, true);
    }
    
    if (lvgl_view_sched.transform_type == lvgl_view_tr_zoom2) {
        
        lv_canvas_transform(lvgl_view_sched.view_cvs, &lvgl_view_sched.view_sps_c, 0, zoom_c,
            lvgl_view_sched.point_c.x, lvgl_view_sched.point_c.y,
            lvgl_view_sched.view_sps_c.header.w / 2, lvgl_view_sched.view_sps_c.header.h / 2, true);
...
static void lvgl_view_transform_center(void)
{
    ...
    if (lvgl_view_sched.transform_type == lvgl_view_tr_center_in)
        zoom_c = lv_map(lvgl_view_sched.pct, 0, 100, LV_IMG_ZOOM_NONE, LV_IMG_ZOOM_NONE / 8);
    
    if (lvgl_view_sched.transform_type == lvgl_view_tr_center_out)
        zoom_c = lv_map(lvgl_view_sched.pct, 0, 100, LV_IMG_ZOOM_NONE, LV_IMG_ZOOM_NONE * 2);
    ...
    if (lvgl_view_sched.transform_type == lvgl_view_tr_center_in ||
        ...
        if (lvgl_view_sched.transform_type == lvgl_view_tr_center_in)
            zoom_t = lv_map(lvgl_view_sched.pct, 0, 100, LV_IMG_ZOOM_NONE * 2, LV_IMG_ZOOM_NONE);
        if (lvgl_view_sched.transform_type == lvgl_view_tr_center_out)
            zoom_t = lv_map(lvgl_view_sched.pct, 0, 100, LV_IMG_ZOOM_NONE / 8, LV_IMG_ZOOM_NONE);
    ...
    if (lvgl_view_sched.transform_type == lvgl_view_tr_center_inout) {
        ...
        if (lvgl_view_sched.pct < 50) {
            ...
            lv_coord_t zoom_c = lv_map(lvgl_view_sched.pct, 0, 50, LV_IMG_ZOOM_NONE, LV_IMG_ZOOM_NONE / 5);
            lv_obj_set_style_transform_pivot_x(lvgl_view_sched.view_c, w / 2, 0);
            lv_obj_set_style_transform_pivot_y(lvgl_view_sched.view_c, h / 2, 0);
            lv_obj_set_style_transform_zoom(lvgl_view_sched.view_c, zoom_c, 0);
            ...
            lv_coord_t zoom_t = lv_map(lvgl_view_sched.pct, 50, 100, LV_IMG_ZOOM_NONE / 5, LV_IMG_ZOOM_NONE);
            lv_obj_set_style_transform_pivot_x(lvgl_view_sched.view_t, w / 2, 0);
            lv_obj_set_style_transform_pivot_y(lvgl_view_sched.view_t, h / 2, 0);
            lv_obj_set_style_transform_zoom(lvgl_view_sched.view_t, zoom_t, 0);
            lv_obj_set_style_opa(lvgl_view_sched.view_t, opa_t, 0);
...
static void lvgl_view_transform_rotate(void)
{
    ...
    if (lvgl_view_sched.slide_dir == LV_DIR_RIGHT) angle_t -= angle_total;
    if (lvgl_view_sched.slide_dir == LV_DIR_LEFT)  angle_t += angle_total;
...
{
    // 异步只需要发生在动画持续时
    if (!lvgl_view_sched.anim_lock) return;
