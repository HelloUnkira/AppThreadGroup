...
void lvgl_lib_ready(void)
{
    ...
    lv_disp_draw_buf_init(&disp_buf, buffer1, buffer2, LV_DRV_HOR_RES * LV_DRV_VER_RES);
    #else
    static lv_disp_draw_buf_t disp_buf;
    static lv_color_t buffer1[LV_DRV_HOR_RES * LV_DRV_VER_RES] = {0};
    static lv_color_t buffer2[LV_DRV_HOR_RES * LV_DRV_VER_RES] = {0};
    lv_disp_draw_buf_init(&disp_buf, buffer1, NULL, LV_DRV_HOR_RES * LV_DRV_VER_RES);
    #endif
    /* 创建显示设备 */
    ...
    /* 创建输入设备组 */
    lv_group_t *lvgl_def_group = lv_group_create();
    /* 注册输入设备:鼠标,键盘,鼠标滑轮 */
    ...
    lv_indev_set_group(kb_indev, lvgl_def_group);
    lv_indev_set_group(mw_indev, lvgl_def_group);
    lv_group_set_default(lvgl_def_group);
    /* 鼠标贴图:cursor */
