...
static void app_lv_ui_local_anim_handler(void *para, int32_t value)
{
    ...
    const char *img_str = app_lv_pic_str_find(app_lv_ui_res_local->list[0].idx_pic + APP_LV_UI_STAR_ROBIT_OFS);
    ...
    lv_coord_t pic_distance = LV_MIN(LV_HOR_RES, LV_VER_RES) / 2 - pic_diameter / 2 - APP_LV_UI_STAR_ROBIT_SPAN;
    int16_t angle_bse = app_lv_ui_res_local->iter_angle / APP_LV_UI_STAR_ROBIT_UNIT;
    int16_t angle_ofs = app_lv_ui_res_local->iter_angle % APP_LV_UI_STAR_ROBIT_UNIT;
    ...
    for (int16_t angle = 0; angle < 360; angle += APP_LV_UI_STAR_ROBIT_UNIT) {
        lv_coord_t pic_x = ((lv_coord_t)lv_trigo_cos(angle + angle_ofs) * pic_distance) >> LV_TRIGO_SHIFT;
        lv_coord_t pic_y = ((lv_coord_t)lv_trigo_sin(angle + angle_ofs) * pic_distance) >> LV_TRIGO_SHIFT;
        ...
        if (angle == 270) {
            const char *img_str = app_lv_pic_str_find(app_lv_ui_res_local->list[pic_ofs].idx_pic + APP_LV_UI_STAR_ROBIT_CEN);
            lv_img_set_src(app_lv_ui_res_local->img_center, img_str);
        }
    }
}
...
static void app_lv_ui_anim_rectify_handler(void *para, int32_t value)
{
    ...
    if (app_lv_ui_res_local->move_flag)
        return;
    /* 动画是否需要执行 */
    if (app_lv_ui_res_local->iter_angle % APP_LV_UI_STAR_ROBIT_UNIT == 0)
        return;
    /* 角度跳跃式(调整优化) */
...
/*@brief 界面默认事件响应回调
 */
static void app_lv_ui_event_img_center_cb(lv_event_t *e)
{
    switch (lv_event_get_code(e)) {
...
static void app_lv_ui_event_default_redirect(lv_event_t *e)
{
    ...
    case LV_EVENT_KEY: {
        ...
        if (key == APP_LV_EVENT_CLOCKWISE_ANTI) {
            app_lv_ui_res_local->iter_angle -= 1;
            app_lv_ui_res_local->iter_way    = -1;
            /* 手动刷新动画 */
            app_lv_ui_local_anim_handler(NULL, 0);
...
void app_lv_ui_star_robit_show(void *scene)
{
    if (app_lv_ui_res_local == NULL) {
        ...
        /* 创建图片资源控件组 */
        APP_SYS_ASSERT(360 % APP_LV_UI_STAR_ROBIT_UNIT == 0);
        app_lv_ui_res_local->img_list = lv_mem_alloc(sizeof(lv_obj_t *) * (360 / APP_LV_UI_STAR_ROBIT_UNIT));
        for (uint8_t idx = 0; idx < 360 / APP_LV_UI_STAR_ROBIT_UNIT; idx++) {
        ...
        app_lv_ui_res_local->img_center = lv_img_create(app_lv_ui_res_local->scene);
        app_lv_style_object(app_lv_ui_res_local->img_center);
        lv_obj_set_style_bg_opa(app_lv_ui_res_local->img_center, LV_OPA_TRANSP, 0);
        lv_obj_add_flag(app_lv_ui_res_local->img_center, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_add_event_cb(app_lv_ui_res_local->img_center, app_lv_ui_event_img_center_cb, LV_EVENT_ALL, NULL);
        lv_obj_center(app_lv_ui_res_local->img_center);
        /* 初始化拨正动画 */
