...
static void app_lv_ui_anim_rectify_handler(void *para, int32_t value)
{
    ...
    lv_coord_t angle_abs  = app_sys_mabs(app_lv_ui_res_local->iter_angle, APP_LV_UI_THUMB_WHEEL_UNIT);
...
/*@brief 界面默认事件响应回调
 */
static void app_lv_ui_event_btn_txt_cb(lv_event_t *e)
{
    switch (lv_event_get_code(e)) {
...
void app_lv_ui_thumb_wheel_show(void *scene)
{
    if (app_lv_ui_res_local == NULL) {
        ...
        lv_obj_add_event_cb(btn, app_lv_ui_event_btn_txt_cb, LV_EVENT_ALL, NULL);
        lv_obj_center(btn);
